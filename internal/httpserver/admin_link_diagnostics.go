...
func (s *Server) handleAdminLinkDiagnosticsSend() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		queryEmail := strings.TrimSpace(r.FormValue("email"))
		redirect := "/admin/link-diagnostics?email=" + url.QueryEscape(queryEmail)
...
func (s *Server) loadLinkDiagnostics(ctx context.Context, data *linkDiagPageData) error {
	...
	rows, err := s.DB.Query(ctx, `
		SELECT c.id, c.full_name, c.email, cl.name, t.name,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.ID, &row.FullName, &row.Email, &row.ClubName, &row.TeamName, &row.ActiveFrom, &row.ActiveTo, &row.IsActive); err != nil {
			return err
		}
	...
	for sendRows.Next() {
		...
		if err := sendRows.Scan(&row.CreatedAt, &row.SeasonName, &row.WeekNumber); err != nil {
			return err
		}
	...
	auditRows, err := s.DB.Query(ctx, `
		...
		FROM audit_logs
		WHERE entity_type IN ('captain', 'team')
		  AND (
		      entity_id = ANY($1::bigint[])
		      OR metadata::text ILIKE '%' || $2 || '%'
		  )
		ORDER BY created_at DESC
		LIMIT 20
	`, int32SliceToInt64(captainIDs), filter)
	if err != nil {
		return err
	}
...
func renderLinkDiagnosticsPage(w http.ResponseWriter, csrfToken string, data linkDiagPageData) {
  ...
  <div class="col-md-6"><input type="email" class="form-control" name="email" placeholder="Captain email address" value="%s" required></div>
  <div class="col-auto"><button class="btn btn-primary" type="submit">Check</button></div>
	...
	for _, row := range data.Captains {
		...
		fmt.Fprintf(w, `<tr><td>%s<div class="text-muted small">%s</div></td><td>%s<br><span class="text-muted small">%s</span></td><td>%s</td><td>%s</td><td><form method="POST" action="/admin/link-diagnostics/send" class="d-inline" onsubmit="return confirm('Send a fresh link to %s? Older general links for this captain/week will be revoked.')"><input type="hidden" name="csrf_token" value="%s"><input type="hidden" name="email" value="%s"><input type="hidden" name="captain_id" value="%d"><button class="btn btn-sm btn-warning" type="submit">Send fresh link</button></form></td></tr>`,
...
func renderLinkDiagSends(w http.ResponseWriter, rows []linkDiagSend) {
	fmt.Fprint(w, `<div class="card shadow-sm mb-4"><div class="card-header fw-semibold">Magic-Link Send Log</div><div class="table-responsive"><table class="table table-sm table-hover mb-0"><thead><tr><th>Sent/logged</th><th>Season</th><th>Week</th></tr></thead><tbody>`)
	if len(rows) == 0 {
		fmt.Fprint(w, `<tr><td colspan="3" class="text-center text-muted py-3">No send log rows found.</td></tr>`)
	}
	for _, row := range rows {
		fmt.Fprintf(w, `<tr><td>%s</td><td>%s</td><td>%d</td></tr>`, row.CreatedAt.Format("02 Jan 15:04"), escapeHTML(row.SeasonName), row.WeekNumber)
	}
	fmt.Fprint(w, `</tbody></table></div></div>`)
}
