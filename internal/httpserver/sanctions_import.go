...
func (s *Server) handleAdminSanctionImports() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `<main class="container py-4"><div class="d-flex flex-column flex-sm-row justify-content-between gap-2 mb-3"><div><h1 class="h2">Import legacy bans &amp; cards</h1><p class="text-muted mb-0">Uploads are stored unchanged and hashed. Rows remain pending until reviewed; this screen never guesses team, player, or rule mappings.</p></div><a class="btn btn-outline-primary align-self-sm-start" href="https://sanctions.gmcl.co.uk/" target="_blank" rel="noopener">View public register</a></div>`)
		...
		fmt.Fprintf(w, `<form method="POST" action="/admin/cases/imports/live" class="card card-gmcl mb-4"><input type="hidden" name="csrf_token" value="%s"><div class="card-body"><h2 class="h5">Get the existing live bans and cards</h2><p class="mb-0">Fetches the three unique public datasets from <a href="%s" target="_blank" rel="noopener">the existing live bans page</a>: individual bans, team cards, and team/cup bans. No export file is required.</p></div><div class="card-footer"><button class="btn btn-primary w-100 w-sm-auto">Fetch and stage live register</button></div></form><form method="POST" action="/admin/cases/imports" enctype="multipart/form-data" class="card mb-4"><input type="hidden" name="csrf_token" value="%s"><div class="card-body row g-3"><div class="col-12"><h2 class="h5 mb-0">Upload another source export</h2></div><div class="col-md-4"><label class="form-label">Source name</label><input class="form-control" name="source_name" required placeholder="Google penalties sheet"></div><div class="col-md-4"><label class="form-label">Source URL</label><input class="form-control" type="url" name="source_url"></div><div class="col-md-4"><label class="form-label">CSV export</label><input class="form-control" type="file" name="file" accept=".csv,text/csv" required></div></div><div class="card-footer"><button class="btn btn-outline-primary w-100 w-sm-auto">Store and stage uploaded rows</button></div></form><div class="alert alert-info"><strong>Staged rows are not public yet.</strong> Use <em>Review &amp; apply</em> for each batch below to create the published sanctions. Rows that cannot be matched remain as exceptions for correction.</div><div class="table-responsive"><table class="table responsive-cards align-middle"><thead><tr><th>Source</th><th>Action</th><th>ID</th><th>File</th><th>SHA-256</th><th>Rows</th><th>Exceptions</th><th>Status</th><th>Uploaded</th></tr></thead><tbody>`, csrf, liveSanctionsRegisterURL, csrf)
		for rows.Next() {
			...
			if rows.Scan(&id, &source, &file, &sum, &status, &at, &rowsN, &exceptions) == nil {
				fmt.Fprintf(w, `<tr><td data-label="Source"><a href="/admin/cases/imports/%d"><strong>%s</strong></a></td><td data-label="Action"><a class="btn btn-sm btn-primary text-nowrap" href="/admin/cases/imports/%d">Review &amp; apply</a></td><td data-label="ID">%d</td><td data-label="File">%s</td><td data-label="SHA-256"><code>%s</code></td><td data-label="Rows">%d</td><td data-label="Exceptions">%d</td><td data-label="Status">%s</td><td data-label="Uploaded">%s</td></tr>`, id, escapeHTML(source), id, id, escapeHTML(file), escapeHTML(sum[:minInt(12, len(sum))]), rowsN, exceptions, escapeHTML(status), at.In(s.LondonLoc).Format("02 Jan 2006 15:04"))
			}
		}
