...
func (s *Server) handleAdminPortalCasesGet() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, item := range cases {
			if access.CanAccessCase(item.ClubID, item.Category, item.CompetitionID) {
				filteredCases = append(filteredCases, item)
			}
		}
		cases = filteredCases
		var requests []portal.AdminOperationalRequest
		var contacts []portal.AdminClubContact
...
<div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4"><div><p class="text-uppercase text-muted small mb-1">Club operations</p><h1 class="h2">Portal work queue</h1><p class="text-muted">Club-visible correspondence is kept separate from internal GMCL notes. Email remains the official record during the pilot.</p></div><div class="d-flex flex-wrap gap-2"><a class="btn btn-primary" href="/admin/portal/messages/new">New club message</a>`)
		if access.SuperAdmin {
			fmt.Fprint(w, `<a class="btn btn-outline-primary" href="/admin/portal">Pilot controls</a>`)
		}
		...
		if access.SuperAdmin {
			fmt.Fprint(w, `<section class="card shadow-sm mb-4"><div class="card-header"><strong>Operational reviews</strong></div><div class="table-responsive"><table class="table table-striped align-middle mb-0"><thead><tr><th>Club</th><th>Request</th><th>Status</th><th>Reference</th><th>Human decision</th></tr></thead><tbody>`)
			if len(requests) == 0 {
...
func (s *Server) handleAdminPortalCaseReply() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		staffRole, ok := access.RoleForCase(
			detail.ClubID,
			detail.Category,
			detail.CampaignCompetitionID,
		)
		...
		messageID, err := s.PortalStore.AddAdminCaseMessage(
			ctx,
			caseID,
			adminIDForRequest(r),
			staffRole,
		...
		status := "replied"
		recipients, recipientErr := s.PortalStore.ListCaseOfficialRecipients(ctx, caseID)
		if recipientErr != nil {
			slog.Warn("load portal case reply recipients failed", "case_id", caseID, "error", recipientErr)
			status = "email-pending"
			_ = s.PortalStore.MarkMessageEmailDelivery(ctx, messageID, false, recipientErr.Error())
		} else if len(recipients) > 0 {
			body := fmt.Sprintf("GMCL replied to portal case %s.\n\nSent by: %s (%s)\n\n%s\n\nOpen the portal: %s/portal/messages/%s", detail.Subject, access.DisplayName, portal.StaffRoleLabel(staffRole), strings.TrimSpace(r.FormValue("body")), publicBaseURL(r), caseID)
			var sendErr error
			for _, recipient := range recipients {
				if err := email.NewFromEnv().SendSensitive(recipient, "GMCL portal: "+detail.Subject, body); err != nil {
					sendErr = err
					break
				}
			}
			if sendErr != nil {
				status = "email-pending"
				_ = s.PortalStore.MarkMessageEmailDelivery(ctx, messageID, false, sendErr.Error())
			} else {
...
func (s *Server) handleAdminPortalMessageEmailRetry() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		detail, err := s.PortalStore.LoadAdminMessageCase(ctx, delivery.CaseID)
		if err != nil {
			http.Error(w, "case not found", http.StatusNotFound)
			return
		}
		...
		campaignDeliveries, err := s.PortalStore.ListRetryableCampaignDeliveries(ctx, messageID)
		if err != nil {
			http.Error(w, "could not load email deliveries", http.StatusInternalServerError)
			return
		}
		if len(campaignDeliveries) > 0 {
			failed := 0
			for _, item := range campaignDeliveries {
...
You can reply or acknowledge in the portal. Email remains the official GMCL communication record during the pilot.`,
					item.SenderName,
					portal.StaffRoleLabel(item.SenderRole),
					item.ClubName,
					portal.RecipientRoleLabel(item.RecipientRole),
					item.Body,
					publicBaseURL(r),
					item.CaseID,
				)
				sendErr := email.NewFromEnv().SendSensitive(
					item.RecipientEmail,
					"GMCL: "+item.Subject,
					body,
				)
				...
				if err := s.PortalStore.CompleteCampaignDelivery(
					ctx,
					item.ID,
					sendErr == nil,
					deliveryError,
					...
					failed++
					slog.Error("record retried portal campaign delivery failed",
						"delivery_id", item.ID, "error", err)
				}
			}
			status := "replied"
			if failed > 0 {
		...
		recipients := []string{portalGMCLCommsEmail()}
		...
		if delivery.AuthorKind == "gmcl_admin" {
			recipients, err = s.PortalStore.ListCaseOfficialRecipients(ctx, delivery.CaseID)
