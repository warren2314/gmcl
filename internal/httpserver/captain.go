...
`, token)
		...
		if err != nil {
			log.Printf("[magic-link] validate failed: %v | ip=%s ua=%q", err, r.RemoteAddr, r.UserAgent())
			http.Error(w, "link invalid or expired", http.StatusBadRequest)
			return
		}
		log.Printf("[magic-link] validated: captain_id=%d | ip=%s ua=%q", mt.CaptainID, r.RemoteAddr, r.UserAgent())
