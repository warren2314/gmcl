...
type OutcomeDraftVersion struct {
	DecisionID int64
	DraftID    int64
	Audience   string
}
...
func (s *Service) SaveOutcomeDraft(ctx context.Context, caseID int64, audience, subject, body string, actor Actor, expected ...OutcomeDraftVersion) (OutcomeDraft, error) {
	if actor.ID == nil {
	...
	if len(expected) > 0 && (expected[0].DecisionID != current.DecisionID || expected[0].DraftID != current.ID) {
		return OutcomeDraft{}, errors.New("this notice has changed; reload before editing")
	}
	body = canonicalOutcomeBody(body)
	...
	var status string
	var ownerID *int32
	if err = tx.QueryRow(ctx, `SELECT d.id,c.status,c.assigned_admin_id FROM sanction_cases c JOIN sanction_decision_revisions d ON d.case_id=c.id AND d.status='proposed' WHERE c.id=$1 ORDER BY d.revision DESC LIMIT 1 FOR UPDATE OF c`, caseID).Scan(&decisionID, &status, &ownerID); err != nil {
		return OutcomeDraft{}, err
	}
	...
	if err = tx.QueryRow(ctx, `SELECT
		EXISTS(SELECT 1 FROM sanction_case_events WHERE case_id=$1 AND event_type='decision_sent_for_approval' AND metadata->>'decision_revision_id'=($2::bigint)::text),
		EXISTS(SELECT 1 FROM admin_users WHERE id=$3 AND is_active),
		COALESCE((SELECT id FROM sanction_correspondence_revisions WHERE case_id=$1 AND decision_revision_id=$2 AND audience=$4 AND status='draft' ORDER BY revision DESC,id DESC LIMIT 1),0)`, caseID, decisionID, *actor.ID, audience).Scan(&sent, &active, &latestID); err != nil {
		return OutcomeDraft{}, err
	}
	if !active || !CanAmendProposedDecision(status, ownerID, actor.ID, sent) {
		return OutcomeDraft{}, errors.New("only the active case owner can edit notices before submission for approval")
	}
